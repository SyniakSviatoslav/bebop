...
impl Delegation {
    /// Canonical bytes that `issued_by` signs. Only the authorization-relevant
    /// fields — not the signature itself — are included, so tampering with any
    /// field invalidates the signature.
    pub fn canonical_bytes(&self) -> CapResult<Vec<u8>> {
        // Sign a fixed-shape struct (not the whole self) to avoid the signature
        // field participating in its own digest. serde_json gives a deterministic
        // field order.
        #[derive(Serialize)]
        struct Wire<'a> {
            issued_by: &'a [u8; 32],
            ...
            resource: &'a Resource,
            action: &'a Action,
            effect_resource: &'a Resource,
            effect_action: &'a Action,
            expiry: &'a u64,
            nonce: &'a [u8; 8],
        }
        let wire = Wire {
            ...
            effect_resource: &self.effect.resource,
            effect_action: &self.effect.action,
            expiry: &self.expiry,
            nonce: &self.nonce,
        };
        serde_json::to_vec(&wire).map_err(|_| CapError::Encode)
    }
